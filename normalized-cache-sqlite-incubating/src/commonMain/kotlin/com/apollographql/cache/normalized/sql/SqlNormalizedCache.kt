...
class SqlNormalizedCache internal constructor(
  ...
  /**
   * Update records.
   * When using the [DefaultRecordMerger], the [records] fields are directly upserted into the db.
   * Otherwise, the [records] are merged programmatically using the given [recordMerger], requiring to load the existing records from
   * the db first.
   */
  private fun internalUpdateRecords(records: Collection<Record>, cacheHeaders: CacheHeaders, recordMerger: RecordMerger): Set<String> {
    ...
    return if (recordMerger is DefaultRecordMerger) {
      ...
      recordDatabase.transaction {
        val existingRecords = recordDatabase.selectRecords(records.map { it.key }).associateBy { it.key }
        records.flatMap { record ->
          ...
          if (existingRecord == null) {
            ...
            val (mergedRecord, changedKeys) = recordMerger.merge(existing = existingRecord, incoming = record.withDates(receivedDate = receivedDate, expirationDate = expirationDate))
